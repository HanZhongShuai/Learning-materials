...
@interface RCRequestTask ()<NSURLConnectionDataDelegate, NSURLSessionDataDelegate>
...
@implementation RCRequestTask

- (instancetype)init {
    if (self = [super init]) {
        [RCFileHandle createTempFile];
    }
    return self;
}
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data {
    if (self.cancel) return;
    [RCFileHandle writeTempFileData:data];
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    if (self.cancel) {
        ...
        if (error) {
            ...
            //可以缓存则保存文件
            if (self.cache) {
                [RCFileHandle cacheTempFileWithFileName:[NSString fileNameWithURL:self.requestURL]];
            }
