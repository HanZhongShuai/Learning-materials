...
@interface RCResourceLoader ()

@property (nonatomic, strong) NSMutableArray * requestList;
@property (nonatomic, strong) RCRequestTask * requestTask;

@end

@implementation RCResourceLoader
...
- (void)requestTaskDidFinishLoadingWithCache:(BOOL)cache {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(loader:finishLoadingWithCacheFinished:)]) {
        [self.delegate loader:self finishLoadingWithCacheFinished:cache];
    }
}
...
- (void)newTaskWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest cache:(BOOL)cache {
    ...
    self.requestTask = [[RCRequestTask alloc]init];
...
- (BOOL)finishLoadingWithLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    [loadingRequest.dataRequest respondWithData:[RCFileHandle readTempFileDataWithOffset:requestedOffset - self.requestTask.requestOffset length:respondLength]];
