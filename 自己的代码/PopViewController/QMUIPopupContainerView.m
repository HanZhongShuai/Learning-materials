...
- (void)setSourceRect:(CGRect)sourceRect
{
    ...
    if (self.isShowing) {
        [self layoutWithTargetRect:_sourceRect];
    }
}
...
- (void)updateLayout {
    ...
    else if (self.sourceBarItem && [self.sourceBarItem respondsToSelector:@selector(view)]) {
        view = [self.sourceBarItem valueForKey:@"view"];
    }
    
    if (view) {
        view = self.sourceView;
        if (!view.window || !self.superview) return;
        UIView *convertToView = self.popupWindow ? UIApplication.sharedApplication.delegate.window : self.superview;// 对于以 window 方式显示的情况，由于横竖屏旋转时，不同 window 的旋转顺序不同，所以可能导致 sourceBarItem 所在的 window 已经旋转了但 popupWindow 还没旋转（iOS 11 及以后），那么计算出来的坐标就错了，所以这里改为用 UIApplication window
        CGRect rect = [view convertRect:view.bounds toView:convertToView];
...
- (void)layoutWithTargetRect:(CGRect)targetRect {
    ...
    if (self.isVerticalLayoutDirection) {
        ...
        if (!canShowAtAbove && !canShowAtBelow) {
            ...
            self.maximumHeight = MAX(self.minimumHeight, MAX(fabs(maximumHeightAbove), fabs(maximumHeightBelow)));
        ...
        if (!canShowAtLeft && !canShowAtRight) {
            ...
            self.maximumWidth = MAX(self.minimumWidth, MAX(fabs(maximumWidthLeft), fabs(maximumWidthRight)));
...
+ (void)setDefaultAppearance {
    ...
    appearance.maskViewBackgroundColor = UIColorMakeWithRGBA(0, 0, 0, .35f);
    appearance.highlightedBackgroundColor = nil;
    appearance.shadowColor = UIColorMakeWithRGBA(0, 0, 0, .2);
    appearance.borderColor = UIColorMake(198, 198, 198);
